...
compile string =
    let parts = divideIntoParts string [] []
    in Template (length parts) parts
  where
    divideIntoParts :: String -> String -> [String] -> [String]
    divideIntoParts ('%':'%':xs) part acc =
        divideIntoParts xs [] (reverse part : acc)
    divideIntoParts ('%':xs) part acc = divideIntoParts xs ('%' : part) acc
    divideIntoParts (x:xs) part acc = divideIntoParts xs (x : part) acc
    divideIntoParts [] part acc = reverse $ reverse part : acc
...
unsafeInterpolate (Template n parts) args =
    ...
    -- Since there is no argument in front of the first part, an empty string is inserted.
        else concat $ concat $ zipWith (\x y -> [x, y]) ([] : args) parts
