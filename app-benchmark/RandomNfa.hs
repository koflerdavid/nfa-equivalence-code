...
randomNfa n cs transitionDensity acceptanceProbability = do
    let p = transitionDensity / fromIntegral n
        stateInputPairs = [(q, c) | q <- [0 .. n], c <- List.sort cs]
    transitions <-
        forM stateInputPairs $ \q_c -> do
            states <- pickStates p
...
prunedNfa nfa =
    let transitions = Map.toList (nfaTransitions nfa)
        reachableStates = getReachableStates [0] transitions
        reachableTransitions =
            filter (\((p, _), _) -> p `ISet.member` reachableStates) transitions
    in nfa
       { nfaTransitions = Map.fromList reachableTransitions
       , nfaAcceptingStates =
             nfaAcceptingStates nfa `ISet.intersection` reachableStates
       }
