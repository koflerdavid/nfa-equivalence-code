        ...
        then transitionTable
                 ...
                 body_ $ do
                     p_ $ do
                         ...
                         code_ $ toHtml . toMinimallyQuotedString $ regex
                     p_ $ do
  ...
  where
    ...
    transitionTableRow (r, ts) =
        tr_ $ do
            ...
            td_ $ toHtml . toMinimallyQuotedString $ r
            td_ $
                if matchesEmptyWord r
                    then "*"
                    else " "
            forM_ (Map.elems ts) $ td_ . toHtml . toMinimallyQuotedString -- Map.elems yields ascending order
