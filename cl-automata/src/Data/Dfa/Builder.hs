...
addTransition from input to dfa =
    case transitionMap dfa Map.!? (from, input) of
        Just existingDestinationState
            | existingDestinationState == to ->
                Right (dfa, DfaState (Just from), DfaState (Just to))
...
withAcceptingStates stateList = snd . withAcceptingStatesUnsafe (mapMaybe toStateNumber stateList)
...
allocateState :: Dfa c -> Int -> DfaState
allocateState _dfa = DfaState . Just
