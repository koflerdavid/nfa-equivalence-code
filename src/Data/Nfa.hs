...
runNfa nfa initialStates = ISet.toList . Foldable.foldl (nfaStep nfa) (ISet.fromList initialStates)

nfaStep :: (Ord c) => Nfa c -> IntSet -> c -> IntSet
nfaStep nfa qs input =
    ISet.unions $ List.map (\q -> Map.findWithDefault ISet.empty (q, input) transitions) $ ISet.toList qs
    where
