...
data Dfa c = Dfa ISet.IntSet (Map.Map (Int, c) Int)
    deriving (Eq, Show)
...
dfaAlphabet (Dfa _ transitions) =
    Set.map snd . Map.keysSet $ transitions
...
dfaAcceptingStates (Dfa acceptingStates _) =
    acceptingStates
...
buildDfa finalStates transitions =
    if Map.size transitionMap < length transitions then Nothing else Just dfa
  where
...
translateDfaStates (Dfa acceptingStates transitions) offset =
  ...
  where
    ...
    transitions' = Map.map (+ offset) . Map.mapKeys (\(q, c) -> (q + offset, c)) $ transitions
