...
data Regex c = Atom c
             | Optional (Regex c)
             | Asterisk (Regex c)
             | forall identifier. (Ord identifier, Show identifier) =>
                                  Capture (Regex c) (Maybe identifier)
             | Alternative [Regex c]
             | Sequence [Regex c]

deriving instance Show c => Show (Regex c)

alphabet :: Ord c => Regex c -> Set c
alphabet regex = execState (computeAlphabet regex) (Set.empty)
  where
    computeAlphabet r = case r of
        Atom c -> modify (Set.insert c)
