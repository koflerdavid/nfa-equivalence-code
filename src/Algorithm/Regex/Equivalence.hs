...
getDifferences regex1 regex2 =
    ...
    in List.map reverseWords . Data.Foldable.toList $ witnesses
  where
...
check ::
    ...
    => [c]
    -> FifoQueue (Witness c)
    -> EquivT' s (Regex c) (Writer (Seq (Witness c))) ()
    ...
    (flip . maybe) (return ()) (Q.pop queue) $ \(constraint@(w, r1, r2), queue') -> do
        ...
        if alreadyEqual || r1 == r2
            then Equiv.equate r1 r2 >> check combinedAlphabet queue'
                     ...
                     then recordMismatch constraint >>
                          check combinedAlphabet queue'
                     else do
                         Equiv.equate r1 r2
                         let makeConstraint c =
                                 (c : w, derive c r1, derive c r2)
                             constraints =
                                 List.map makeConstraint combinedAlphabet
