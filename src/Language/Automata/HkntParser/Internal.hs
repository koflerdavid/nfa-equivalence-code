...
transition = do
  ...
  return [(origin, label, destination) | origin <- origins, label <- labels, destination <- destinations]
...
identifier = token (show . snd) fst (\case (_, Identifier name) -> Just name; _ -> Nothing)

acceptKeyword :: Parsec [(SourcePos, Token)] s ()
acceptKeyword = token (show . snd) fst (\case (_, Accept) -> Just (); _ -> Nothing)

checkKeyword :: Parsec [(SourcePos, Token)] s ()
checkKeyword = token (show . snd) fst (\case (_, Check) -> Just (); _ -> Nothing)

colonToken :: Parsec [(SourcePos, Token)] s ()
colonToken = token (show . snd) fst (\case (_, Colon) -> Just (); _ -> Nothing)

equalsOperator :: Parsec [(SourcePos, Token)] s ()
equalsOperator = token (show . snd) fst (\case (_, Equals) -> Just (); _ -> Nothing)

greaterEqualsOperator :: Parsec [(SourcePos, Token)] s ()
greaterEqualsOperator = token (show . snd) fst (\case (_, GreaterEquals) -> Just (); _ -> Nothing)

transitionArrow :: Parsec [(SourcePos, Token)] s [Char]
transitionArrow = token (show . snd) fst (\case (_, Arrow labels) -> Just labels; _ -> Nothing)

newlineToken :: Parsec [(SourcePos, Token)] s ()
newlineToken = token (show . snd) fst (\case (_, Newline) -> Just (); _ -> Nothing)
