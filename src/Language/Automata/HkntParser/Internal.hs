...
acceptingStates =
    option [] $ acceptKeyword *> colonToken *> many1 identifier <* newlineToken
...
check =
    checkKeyword *> colonToken *>
    ((,,) <$> many1 identifier <*> operation <*> many1 identifier)
  where
    operation =
        equalsOperator $> Equivalence <|> greaterEqualsOperator $> Inclusion
