...
spec = do
    describe "hkntTokenizer" $ do
        let cases = [ ( "a -a-> b c",
                        [ Identifier "a", Arrow "a", Identifier "b", Identifier "c", Newline ])
                    , ( "a b -c-> d",
                        [ Identifier "a", Identifier "b", Arrow "c", Identifier "d", Newline ])
                    , ( "a -b+c+d-> e f",
                        [ Identifier "a", Arrow "bcd", Identifier "e", Identifier "f", Newline ])
                    , ("accept: a b",
                        [ Accept, Colon, Identifier "a", Identifier "b", Newline ])
                    , ( "accept:a b  c ",
                        [ Accept, Colon, Identifier "a", Identifier "b", Identifier "c", Newline ])
                    , ( "check: a b => c",
                        [ Check, Colon, Identifier "a", Identifier "b", GreaterEquals, Identifier "c", Newline])
                    , ( "check: a = b c",
                        [ Check, Colon, Identifier "a", Equals, Identifier "b", Identifier "c", Newline])
                    ]
        forM_ cases $
            \(input, expected) -> do
        ...
        it "should tokenize the example from the introduction of the HKNT paper" $ do
            ...
            tokensFromInput introductionInHkntFormat `shouldBe`
                Right [ Identifier "x", Arrow "a", Identifier "y", Newline,
