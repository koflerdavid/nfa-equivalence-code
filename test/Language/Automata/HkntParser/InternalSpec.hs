module Language.Automata.HkntParser.InternalSpec (main, spec) where
...
spec = do
  describe "transition parser" $ do
    let cases = [
                ...
                , ( "a -a+b-> b", [("a", 'a', "b"), ("a", 'b', "b")] )
                , ( "a c -a-> b", [("a", 'a', "b"), ("c", 'a', "b")] )
                , ( "a c -a+b-> b", [("a", 'a', "b"), ("a", 'b', "b"),
                                     ("c", 'a', "b"), ("c", 'b', "b")] )
                , ( "a -a+b-> b c", [("a", 'a', "b"), ("a", 'a', "c"),
                                     ("a", 'b', "b"), ("a", 'b', "c")] )
                , ( "a d -a+b-> b c", [("a", 'a', "b"), ("a", 'a', "c"),
                                       ("a", 'b', "b"), ("a", 'b', "c"),
                                       ("d", 'a', "b"), ("d", 'a', "c"),
                                       ("d", 'b', "b"), ("d", 'b', "c")
                                       ])
...
shouldAccept :: (Eq a, Show a) => Parsec [(SourcePos, Token)] () a -> String -> a -> Expectation
shouldAccept parser input expectedResult = tokeniseAndParse parser "<input>" input `shouldBe` Right expectedResult
